  ...
  <div class="ay-x-input ay-cell" :class="{'ay-cell_warn': !valid}">
    ...
    <div class="ay-cell__bd ay-cell__primary">
      ...
      <input
        ...
        @blur="blur"
        v-el:input/>
      <div v-show="(alertmessageshow&&!valid) && alertmessageswitch" class="x-input-alert-message">{{alertmessage}}
      </div>
    </div>
    <div class="ay-cell__ft">
      <icon type="clear" v-show="!equalWith && showClear && value && !readonly && !disabled"
            @click.native="clear"></icon>

      <icon class="ay-input-icon" type="warn" :title="!valid ? firstError : ''"
            v-show="!novalidate && !equalWith && ((touched && !valid && firstError) || (forceShowError && !valid && firstError))"></icon>
      <icon class="ay-input-icon" type="warn"
