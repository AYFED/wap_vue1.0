...
var sFtv = new Array("0101*元旦", "0214 情人节", "0308 妇女节", "0312 植树节", "0401 愚人节", "0501 劳动节", "0504 青年节", "0512 护士节", "0601 儿童节", "0701 建党节", "0801 建军节", "0808 父亲节", "0910 教师节", "0928 孔子诞辰", "1001*国庆节", "1006 老人节", "1024 联合国日", "1225 圣诞节");

export function getFestival(newDate) {
    var sDObj = new Date(newDate);
    var llDObj = calendar.solar2lunar(parseInt(sDObj.getFullYear()), parseInt(sDObj.getMonth()) + 1, parseInt(sDObj.getDate()));
    ...
    for (let ii = 0, jj = lFtv.length; ii < jj; ii++)
        ...
        if (lFtv[ii].match(/^(\d{2})(.{2})([\s\*])(.+)$/)) {

            tmp1 = Number(RegExp.$1) - llDObj.lMonth

            tmp2 = Number(RegExp.$2) - llDObj.lDay
    ...
    //2018年春分bug零时性修复
    if(newDate == '2018-03-20'){
        festival = []
    }

    if(newDate == '2018-03-21'){
        festival = ['春分']
    }
