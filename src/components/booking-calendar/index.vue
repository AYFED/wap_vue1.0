<template>
    <div class="booking-calendar" :class="{'is-weekend-highlight': highlightWeekend}">
        <div class="alertContent" v-show="(alertShow&&alertable) || forceshowalert" transition="expand">{{alertText}}
        </div>
        <table>
            <thead v-show="!hideWeekList">
            <tr class="calendar-month-bar">
                <th @click="goToMonth(showMonthList[dk].m-1,showMonthList[dk].y,$index)" v-for="dk in 7"
                    class="{{$index==currentIndex?'weekend':''}}"><span>{{showMonthListNoObject[dk]?showMonthListNoObject[dk]+'月':''}}</span>
                </th>
            </tr>
            <tr class="calendar-weeks-bar" v-show="!tbodyshow">
                <th v-for="(index, week) in weeksList"
                    class="{{($index==0||$index==(weeksList.length-1))?'weekend':''}}"><span>{{week}}</span></th>
            </tr>
            </thead>
...
<script>
    ...
    import {getDays, monthNumBtTow, zero, getFestival} from './util'
    ...
    export default {
        ...
        methods: {
            ...
            replaceText(day, formatDay) {
                var _tmp = getFestival(formatDay)
            ...
            getCurDate(){
                ...
                let curStr = curDate.getFullYear() + "/" + curDate.getMonth() + "/" + curDate.getHours()
                return new Date(curStr)
            },
            render(year, month, v, cdata) {
                let data = getDays({
                    ...
                    month: month,
                    value: this.value,
                    rangeBegin: this.convertDate(this.startDate) || this.convertDate(this.getCurDate()),
