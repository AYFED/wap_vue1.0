<template>
  <div class="ay_cell">
    ...
    <div class="ay_cell_ft" v-show="!readonly" style="font-size:0">
      <a @click="sub" class="ay-number-selector ay-number-selector-sub"
         :class="{'ay-number-disabled':disabledMin}">-</a>
      <input v-model="value" :name="name" class="ay-number-input" :style="{width: width+'px'}" number
             :readonly="!fillable" pattern="[0-9]*" onfocus="if(value=defaultValue){value=''}"
             onKeyUp="value=value.replace(/[^\d]/g,'') "/>
      <a @click="add" class="ay-number-selector ay-number-selector-plus"
         :class="{'ay-number-disabled':disabledMax}">+</a>
    </div>
